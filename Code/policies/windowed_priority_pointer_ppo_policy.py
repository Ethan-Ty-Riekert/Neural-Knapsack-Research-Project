"""windowed_priority_pointer_ppo_policy.py - sb3_contrib-compatible policy
wrapper around WindowedPriorityPointerActorCritic (see
windowed_priority_pointer_policy.py). PREPARED FOR REVIEW, NOT YET WIRED
INTO ANY TRAINING RUN. Mirrors Code/policies/priority_pointer_ppo_policy.py
exactly (same four MaskablePPO-called methods overridden), differing only
in which network class it builds and which constructor kwargs it forwards.
"""
...
class WindowedPriorityPointerMaskableActorCriticPolicy(MaskableActorCriticPolicy):
    def __init__(
        self,
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        self.pointer_net = WindowedPriorityPointerActorCritic(**self._pointer_kwargs)
